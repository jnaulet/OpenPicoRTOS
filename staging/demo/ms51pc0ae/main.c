...
static void out_main(void *priv)
{
    picoRTOS_assert_fatal(priv != NULL, return );

    struct gpio *OUT = (struct gpio*)priv;

    for (;;) {
        gpio_toggle(OUT);
        picoRTOS_sleep(PICORTOS_DELAY_MSEC(500ul));
    }
}
...
int main(void)
{
    ...
    picoRTOS_init();
    (void)ms51pc0ae_init(&ms51);

    /* tick */
    picoRTOS_task_init(&task, tick_main, &ms51.TICK, stack0, PICORTOS_STACK_COUNT(stack0));
    picoRTOS_add_task(&task, picoRTOS_get_next_available_priority());
    /* led */
    picoRTOS_task_init(&task, led0_main, &ms51.D2, stack1, PICORTOS_STACK_COUNT(stack1));
    picoRTOS_add_task(&task, picoRTOS_get_next_available_priority());
    /* led */
    picoRTOS_task_init(&task, led1_main, &ms51.D2, stack2, PICORTOS_STACK_COUNT(stack2));
    picoRTOS_add_task(&task, picoRTOS_get_next_available_priority());
    /* out */
    picoRTOS_task_init(&task, out_main, &ms51.OUT1, stack3, PICORTOS_STACK_COUNT(stack3));
    picoRTOS_add_task(&task, picoRTOS_get_next_available_priority());
    /* uart */
    picoRTOS_task_init(&task, uart_main, &ms51.UART0, stack4, PICORTOS_STACK_COUNT(stack4));
    picoRTOS_add_task(&task, picoRTOS_get_next_available_priority());
