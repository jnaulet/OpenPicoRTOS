 ...
 */
int gpio_atmel_pio_init(struct gpio *ctx, int base, size_t pin)
{
    picoRTOS_assert(pin < (size_t)GPIO_ATMEL_PIO_PIN_COUNT, return -EINVAL);

    ctx->base = (struct GPIO_ATMEL_PIO*)base; // NOLINT
