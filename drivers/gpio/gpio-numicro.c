...
/* Function: gpio_numicro_init
 * Initializes a GPIO
 *
 * Parameters:
 *  ctx - The GPIO to init
 *  port - The GPIO port index [0-3]
 *  pin - The GPIO pin number on that port
 *
 * Returns:
 * 0 if success, -errno otherwise
 */
int gpio_numicro_init(struct gpio *ctx, size_t port, size_t pin)
{
    picoRTOS_assert(port < (size_t)GPIO_NUMICRO_PORT_COUNT, return -EINVAL);
    picoRTOS_assert(pin < (size_t)GPIO_NUMICRO_PIN_COUNT, return -EINVAL);
