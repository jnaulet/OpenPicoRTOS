...
struct UART_ATSAMX7X {
    ...
    volatile uint32_t UART_WMPR;
};

#define UART_CR_REQCLR (1 << 12)
...
#define UART_MR_BRSRCCK   (1 << 12)
#define UART_MR_PAR_M     0x7u
#define UART_MR_PAR(x)    (((x) & UART_MR_PAR_M) << 9)
#define UART_MR_FILTER    (1 << 4)

#define UART_SR_CMD     (1 << 15)
...
#define UART_CMPR_VAL2_M  0xffu
#define UART_CMPR_VAL2(x) (((x) & UART_CMPR_VAL2_M) << 16)
#define UART_CMPR_CMPPAR  (1 << 14)
#define UART_CMPR_CMPMODE (1 << 12)
#define UART_CMPR_VAL1_M  0xffu
#define UART_CMPR_VAL1(x) ((x) & UART_CMPR_VAL1_M)

#define UART_WPMR_WPKEY_M  0xffffffu
#define UART_WPMR_WPKEY(x) (((x) & UART_WPMR_WPKEY_M) << 8)
#define UART_WPMR_WPEN     (1 << 0)

/* Function: uart_atsamx7x_init
 * Initializes an UART
 *
 * Parameters:
