...
/* Function: uart_atsamx7x_usart_init
 ...
 * Always 0
 */
int uart_atsamx7x_usart_init(struct uart *ctx, int base, clock_id_t clkid)
{
...
/* Function: uart_atsamx7x_usart_set_usclks
 * Selects the UART input clock
 *
 * Parameters:
 *  ctx - The UART to configure
 *  usclks - The clock to use
 *
 * Returns:
 * 0 if clock is supported, -EINVAL otherwise
 */
...
int uart_read(struct uart *ctx, char *buf, size_t n)
{
    ...
    while ((size_t)recv != n) {
        if ((ctx->base->US_CSR & US_CSR_RXRDY) == 0)
            break;

        buf[recv++] = (char)ctx->base->US_RHR;
    }
