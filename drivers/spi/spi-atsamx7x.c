...
#define div_ceil(x, y) (((x) + ((y) - 1)) / (y))
    ...
    picoRTOS_assert(cs < (size_t)SPI_ATSAMX7X_CS_COUNT, return -EINVAL);
    picoRTOS_assert(frame_size >= (size_t)SPI_ATSAMX7X_FRAME_SIZE_MIN, return -EINVAL);
    picoRTOS_assert(frame_size <= (size_t)SPI_ATSAMX7X_FRAME_SIZE_MAX, return -EINVAL);

    size_t bits = frame_size - (size_t)SPI_ATSAMX7X_FRAME_SIZE_MIN;
