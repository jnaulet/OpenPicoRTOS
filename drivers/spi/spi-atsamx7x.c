...
struct SPI_ATSAMX7X {
    ...
    volatile uint32_t SPI_CSR[SPI_ATSAMX7X_CS_COUNT];
...
#define SPI_WPMR_WPKEY_M 0xffffffu
#define SPI_WPMR_WKEY(x) (((x) & SPI_WPMR_WPKEY_M) << 8)
#define SPI_WPMR_WPCREN  (1 << 2)
#define SPI_WPMR_WPITEN  (1 << 1)
#define SPI_WPMR_WPEN    (1 << 0)

/* Function: spi_atsamx7x_init
 * Initializes a SPI
 *
 * Parameters:
 *  ctx - The SPI to init
...
/* Function: spi_atsamx7x_set_loopback
...
{
    picoRTOS_assert(cs < (size_t)SPI_ATSAMX7X_CS_COUNT, return -EINVAL);
...
#define div_ceil(x, y) (((x) + ((y) - 1)) / (y))
    ...
    picoRTOS_assert(cs < (size_t)SPI_ATSAMX7X_CS_COUNT, return -EINVAL);
    picoRTOS_assert(frame_nbits >= (size_t)SPI_ATSAMX7X_FRAME_NBITS_MIN, return -EINVAL);
    picoRTOS_assert(frame_nbits <= (size_t)SPI_ATSAMX7X_FRAME_NBITS_MAX, return -EINVAL);

    size_t bits = frame_nbits - (size_t)SPI_ATSAMX7X_FRAME_NBITS_MIN;
...
{
    picoRTOS_assert(settings->cs < (size_t)SPI_ATSAMX7X_CS_COUNT,
                    return -EINVAL);
