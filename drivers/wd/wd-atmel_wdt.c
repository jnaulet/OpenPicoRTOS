...
#define DEFAULT_DIVIDER 128
    uint32_t wdv;
    uint32_t wdt_mr = ctx->base->WDT_MR;
    ...
    wdt_mr &= ~(WDT_MR_WDIDLEHLT | WDT_MR_WDD(WDT_MR_WDD_M) | WDT_MR_WDV(WDT_MR_WDV_M));
    wdt_mr |= (WDT_MR_WDD(wdv) | WDT_MR_WDV(wdv));

    /* can only be written once */
    ctx->base->WDT_MR = wdt_mr;
    return 0;
}
