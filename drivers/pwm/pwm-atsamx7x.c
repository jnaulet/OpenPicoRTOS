...
struct PWM_ATSAMX7X {
    ...
    } PWM_CH[PWM_ATSAMX7X_CHANNEL_COUNT];
};
...
{
    if (!picoRTOS_assert(settings->clock_a_pre < PWM_ATSAMX7X_CLOCK_PRE_COUNT)) return -EINVAL;
    if (!picoRTOS_assert(settings->clock_a_div < PWM_ATSAMX7X_CLOCK_DIV_COUNT)) return -EINVAL;
    if (!picoRTOS_assert(settings->clock_b_pre < PWM_ATSAMX7X_CLOCK_PRE_COUNT)) return -EINVAL;
    if (!picoRTOS_assert(settings->clock_b_div < PWM_ATSAMX7X_CLOCK_DIV_COUNT)) return -EINVAL;
...
static clock_freq_t pwm_atsamx7x_get_freq(struct pwm_sam *ctx, pwm_sam_pwm_cpre_t cpre)
{
    if (!picoRTOS_assert(cpre < PWM_ATSAMX7X_PWM_CPRE_COUNT)) return (clock_freq_t)-EINVAL;

    clock_freq_t ret;

    switch (cpre) {
    case PWM_ATSAMX7X_PWM_CPRE_MCK:             /*@fallthrough@*/
    case PWM_ATSAMX7X_PWM_CPRE_MCK_DIV_2:       /*@fallthrough@*/
    case PWM_ATSAMX7X_PWM_CPRE_MCK_DIV_4:       /*@fallthrough@*/
    case PWM_ATSAMX7X_PWM_CPRE_MCK_DIV_8:       /*@fallthrough@*/
    case PWM_ATSAMX7X_PWM_CPRE_MCK_DIV_16:      /*@fallthrough@*/
    case PWM_ATSAMX7X_PWM_CPRE_MCK_DIV_32:      /*@fallthrough@*/
    case PWM_ATSAMX7X_PWM_CPRE_MCK_DIV_64:      /*@fallthrough@*/
    case PWM_ATSAMX7X_PWM_CPRE_MCK_DIV_128:     /*@fallthrough@*/
    case PWM_ATSAMX7X_PWM_CPRE_MCK_DIV_256:     /*@fallthrough@*/
    case PWM_ATSAMX7X_PWM_CPRE_MCK_DIV_512:     /*@fallthrough@*/
    case PWM_ATSAMX7X_PWM_CPRE_MCK_DIV_1024:
        ret = (clock_freq_t)((size_t)ctx->freq >> cpre);
        break;

    case PWM_ATSAMX7X_PWM_CPRE_CLKA:
        ret = ctx->freq / (clock_freq_t)ctx->clock_a_fact;
        break;

    case PWM_ATSAMX7X_PWM_CPRE_CLKB:
...
/* Function: pwm_atsamx7x_pwm_init
 ...
 * 0 if success, -errno otherwise
 */
int pwm_atsamx7x_pwm_init(struct pwm *ctx, struct pwm_sam *parent, size_t channel)
{
    if (!picoRTOS_assert(channel < (size_t)PWM_ATSAMX7X_CHANNEL_COUNT)) return -EINVAL;

    ctx->parent = parent;
    ctx->ch = &parent->base->PWM_CH[channel];
    ctx->channel = channel;
    ctx->freq = pwm_atsamx7x_get_freq(ctx->pwm, PWM_ATSAMX7X_PWM_CPRE_MCK);
    ctx->ncycles = 0;

    return 0;
}

/* Function: pwm_atsamx7x_pwm_setup
...
{
    if (!picoRTOS_assert(settings->cpre < PWM_ATSAMX7X_PWM_CPRE_COUNT)) return -EINVAL;
    ...
    ctx->freq = pwm_atsamx7x_get_freq(ctx->parent, settings->cpre);
    return 0;
}
